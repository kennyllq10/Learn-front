...
export default {
    ...
    editParams(){
         this.$refs.editFormRef.validate(async valid => {
              ...
              if(!valid) return
              //可以发起修改用户的网络请求
              const {data:res} = await this.$http.put(`categories/${this.cateId}/attributes/${this.editForm.attr_name}`,{attr_name:this.editForm.attr_name,attr_sel:this.activeName})
              if(res.meta.status !== 200) {
